...
export async function showRecipeModal(recipeIdOrObject) {
  try {
    ...
    modal
      .querySelector(".save-recipe-modal")
      ?.addEventListener("click", (e) => {
        saveRecipe(e.target.dataset.recipeId, e.target);
      });

    modal
      .querySelector(".add-to-meal-modal")
      ?.addEventListener("click", (e) => {
...
async function addToMealPlan(recipeData) {
  try {
    ...
    if (typeof recipeData === "string") {
      ...
      recipeData = await apiManager.getRecipeInformation(recipeId, {
        includeNutrition: false,
      });
    }
    ...
    showMessage(
      `✅ "${recipeData.title}" queued for meal plan! Go to the Meal Planner to add it.`,
      "success",
    );
