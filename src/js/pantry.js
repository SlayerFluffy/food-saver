...
class PantryManager {
  ...
  setupEventListeners() {
    ...
    if (sortSelect) {
      sortSelect.addEventListener("change", (e) =>
        this.sortPantry(e.target.value),
      );
    }
  ...
  updatePantryDisplay() {
    ...
    tbody.innerHTML = this.pantryItems
      .map((item) => this.createTableRowHTML(item))
      .join("");
  ...
  addSelectedToShoppingList() {
    ...
    const selectedItemsList = this.pantryItems.filter((item) =>
      this.selectedItems.has(item.id),
    );
    const itemsStr = selectedItemsList
      .map((item) => `${item.quantity} ${item.unit} ${item.name}`)
      .join(", ");

    this.showMessage(
      `Added ${this.selectedItems.size} item(s) to shopping list: ${itemsStr}`,
      "success",
    );

    // Add items to shopping list via auth system
    const shoppingItems = selectedItemsList.map((item) => ({
  ...
  editItem(itemId) {
    ...
    const currentExpiryDate = item.expiryDate
      ? item.expiryDate.split("T")[0]
      : "";
    const newExpiryDate = prompt(
      `Edit expiry date for ${newName}: (YYYY-MM-DD, leave blank for none)`,
      currentExpiryDate,
    );
