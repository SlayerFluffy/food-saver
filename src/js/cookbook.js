...
class CookbookManager {
    ...
    setupEventListeners() {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', (e) => this.filterCookbook(e.target.value));
        }

        // Sort functionality
        const sortSelect = qs('#cookbook-sort');
        if (sortSelect) {
            sortSelect.addEventListener('change', (e) => this.sortCookbook(e.target.value));
        }
    ...
    updateCookbookDisplay() {
        ...
        cookbookContainer.innerHTML = this.cookbookRecipes.map(recipe => this.createRecipeCardHTML(recipe)).join('');
    ...
    createRecipeCardHTML(recipe) {
        ...
        const tagsDisplay = recipe.tags && recipe.tags.length > 0 
        ...
        return `
            <div class="recipe-card ${recipe.veryHealthy ? 'healthy' : ''}" data-recipe-id="${recipe.id}">
                ...
                <div class="recipe-content">
                    ...
                    ${recipe.notes ? `<div class="recipe-notes">
                ...
                <div class="recipe-actions">
                    ...
                    <button class="btn add-to-planner" data-recipe-id="${recipe.id}">Add to Planner</button>
    ...
    removeRecipe(recipeId) {
        ...
        if (confirm(`Are you sure you want to remove "${recipe.title}" from your cookbook?`)) {
            try {
                ...
                this.cookbookRecipes = this.cookbookRecipes.filter(r => r.id !== recipeId);
                this.updateCookbookDisplay();
    ...
    addToPlanner(recipeId) {
        ...
        // Implement meal planner integration
        this.showMessage(`${recipe.title} added to meal planner!`, 'success');
    ...
    sortCookbook(sortBy) {
        ...
        switch (sortBy) {
            case 'title':
                sortedRecipes = [...this.cookbookRecipes].sort((a, b) => a.title.localeCompare(b.title));
                break;
            case 'time':
                sortedRecipes = [...this.cookbookRecipes].sort((a, b) => (a.readyInMinutes || 999) - (b.readyInMinutes || 999));
                break;
            case 'added':
                sortedRecipes = [...this.cookbookRecipes].sort((a, b) => new Date(b.addedDate) - new Date(a.addedDate));
                break;
            default:
